...
//   yPosition = height/2;
  ...
  rules.set("X", "<1+F>1+<1F>1[+y][-y]<1F>1-<1F>1[+y][-y]");
  rules.set("Y", "<1-F>1-<1F>1[+x][-x]<1F>1+<1F>1[+x][-x]");
  rules.set("F", "<10F[+x][-x][+y][-y]F>10");
...
void rewriteWord() {
  ...
  for (currentCharacter= 0; currentCharacter < existingWord.length(); currentCharacter++) {
    ...
    else if ( existingWord.charAt(currentCharacter) == 'Y') {
      // replace
      newWord += rules.get("Y");
    } 
