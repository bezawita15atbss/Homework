...
//   yPosition = height/2;
  ...
  rules.set("x", "@.95F[s][t]");
  rules.set("s", "-<2F-<2F-<2FF-<2FFF[s][t]");
  rules.set("s", "-<2F[s][t]F[s][t]F[s][t]F[s][t]F>2[s][t]");
  rules.set("s", "-@.95F[s][t]@.95F[s][t]@.95F[s][t]>2");
  rules.set("s", "-@.95F[s][t]@.95F[s][t]>2-@.95F[s][t]");
  rules.set("t", "+<2F+<2F+<2FF+<2FFF[s][t]");
  rules.set("t", "+<2F[s][t]F[s][t]F[s][t]F[s][t]F>2[s][t]");
  rules.set("t", "+@.95F[s][t]@.95F[s][t]@.95F[s][t]>2");
  rules.set("t", "+@.95F[s][t]@.95F[s][t]>2+@.95F[s][t]");
  rules.set("f", "fffff");
...
void rewriteWord() {
  ...
  for (currentCharacter= 0; currentCharacter < existingWord.length(); currentCharacter++) {
    ...
    else if ( existingWord.charAt(currentCharacter) == 's') {
      // replace
      newWord += rules.get("s");
    }
    else if ( existingWord.charAt(currentCharacter) == 't') {
      // replace
      newWord += rules.get("t");
    }
